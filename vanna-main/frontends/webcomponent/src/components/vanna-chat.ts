...
export class VannaChat extends LitElement {
  ...
  @property() title = 'Vanna AI 聊天';
  @property() placeholder = '问我任何问题...';
  ...
  private async _sendMessageInternal(messageText: string): Promise<boolean> {
    ...
    try {
      ...
      this.setStatus('error', '发送消息失败', error instanceof Error ? error.message : '未知错误');
  ...
  private async handleStreamingResponse(request: any) {
    ...
    try {
      ...
      try {
        ...
        this.setStatus('error', '连接失败', '无法连接到服务器');
        throw pollError;
      }
    }
  }
  ...
  private async processChunk(chunk: ChatStreamChunk) {
    ...
    switch (componentType) {
      ...
      case 'error':
        throw new Error(chunk.rich.data?.message || '代理返回未知错误');
        ...
        <div class="chat-main">
          ...
          <div class="chat-messages">
            ...
            <div class="empty-state" id="empty-state">
              ...
              <div class="empty-state-subtitle">在下方输入您的消息开始聊天</div>
            </div>
