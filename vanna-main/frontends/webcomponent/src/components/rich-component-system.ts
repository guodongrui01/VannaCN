    ...
    if (searchable || exportable || filterable) {
      actionsHTML = `
        <div class="dataframe-actions">
          ...
          ` : ''}
          ${exportable ? `
            <button class="export-btn" title="导出为 CSV">📥 导出</button>
          ` : ''}
        </div>
      `;
    }
        ...
        <div class="log-viewer-header">
          ...
          ${searchable ? `
            <div class="log-viewer-search">
              <input type="text" placeholder="搜索日志..." class="log-search-input">
            </div>
          ` : ''}
        </div>
    ...
    container.innerHTML = `
      <div class="artifact-header">
        <div class="artifact-meta">
          <h3 class="artifact-title">${title || '作品'}</h3>
          ${description ? `<p class="artifact-description">${description}</p>` : ''}
          <span class="artifact-type-badge">${artifact_type}</span>
        </div>
        <div class="artifact-controls">
          ${editable ? '<button class="artifact-btn edit-btn" title="编辑">✏️</button>' : ''}
          ${fullscreen_capable ? '<button class="artifact-btn fullscreen-btn" title="全屏">⛶</button>' : ''}
          ${external_renderable ? '<button class="artifact-btn external-btn" title="在外部打开">🔗</button>' : ''}
        </div>
      </div>
        ...
        <div class="artifact-placeholder">
          <div class="placeholder-content">
            ...
            <div class="placeholder-text">
              <strong>${title || '作品'}</strong> 已在外部打开
              <div class="placeholder-type">${artifact_type}</div>
            </div>
            <button class="placeholder-reopen" title="重新打开">↗</button>
          </div>
        </div>
      `;
