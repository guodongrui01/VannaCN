    ...
    async def send_message(
        ...
        *,
        ...
        try:
            ...
            error_description = "处理您的消息时发生意外错误，请重试。"
            if conversation_id:
            ...
            yield UiComponent(
                ...
                simple_component=SimpleTextComponent(
                    text=f"错误：发生意外错误，请重试。{f' (对话 ID: {conversation_id})' if conversation_id else ''}"
                ),
            )
    ...
    async def _send_message(
        ...
        *,
        ...
        if is_starter_request and self.workflow_handler:
            ...
            try:
                ...
                if components:
                    ...
                    yield UiComponent(  # type: ignore
                        rich_component=ChatInputUpdateComponent(
                            placeholder="问一个问题...", disabled=False
                        )
                    )
            ...
            if response.is_tool_call():
                    ...
                    if has_tool_invocation_message_in_chat:
                        ...
                        yield UiComponent(  # type: ignore
                            rich_component=StatusBarUpdateComponent(
                                status="working",
                                message="正在执行工具...",
                                detail=f"正在运行 {len(response.tool_calls or [])} 个工具",
                            )
                        )
                    else:
                ...
                for i, tool_call in enumerate(response.tool_calls or []):
                    ...
                    tool_status_card = StatusCardComponent(
                        title=f"正在执行 {tool_call.name}",
                        status="running",
                        description=f"使用 {len(tool_call.arguments)} 个参数运行工具",
                        icon="⚙️",
                    ...
                    final_description = (
                        f"工具执行成功"
                        if result.success
                        else f"工具执行失败: {result.error or '未知错误'}"
                    )
                ...
                yield UiComponent(  # type: ignore
                    rich_component=ChatInputUpdateComponent(
                        placeholder="问一个跟进问题...", disabled=False
                    )
                )
        ...
        if tool_iterations >= self.config.max_tool_iterations:
            ...
            logger.warning(
                f"工具迭代限制已达到: {tool_iterations}/{self.config.max_tool_iterations}"
            )
            ...
            yield UiComponent(  # type: ignore
                rich_component=StatusBarUpdateComponent(
                    status="warning",
                    message="工具执行次数已达上限",
                    detail=f"在执行 {tool_iterations} 次工具后停止。任务可能未完成。",
                )
            )

            # Provide detailed warning message to user
            warning_message = f"""⚠️ **工具执行次数已达上限**

代理在执行 {tool_iterations} 个工具后停止（达到配置的最大值）。任务可能尚未完全完成。

您可以：
- 让我继续从上次停止的地方继续
- 如果需要更多工具调用，请调整 `max_tool_iterations` 设置
- 将任务分解成更小的步骤"""
            ...
            yield UiComponent(
                ...
                simple_component=SimpleTextComponent(
                    text=f"执行 {tool_iterations} 次后工具限制已达到。任务可能未完成。"
                ),
            )
