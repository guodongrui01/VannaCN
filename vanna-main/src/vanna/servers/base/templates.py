...
<body>
    <div class="max-w-6xl mx-auto p-5">
        ...
        <div class="text-center mb-8">
            <h1 class="text-4xl font-bold text-vanna-navy mb-2 font-serif">Vanna 智能助手</h1>
            <p class="text-lg font-mono font-bold text-vanna-teal mb-4">数据优先的 AI 代理</p>
            <p class="text-slate-600 mb-4">由 Vanna 框架驱动的交互式 AI 助手</p>
        ...
        {('    <div class="bg-vanna-orange/10 border border-vanna-orange/30 rounded-lg p-3 mb-5 text-vanna-orange text-sm font-medium">📦 开发模式：从本地资源加载组件</div>' if dev_mode else "")}

        <!-- Login Form -->
        <div id="loginContainer" class="max-w-md mx-auto mb-10 bg-white p-8 rounded-xl shadow-lg border border-vanna-teal/30">
            <div class="text-center mb-6">
                <h2 class="text-2xl font-semibold text-vanna-navy mb-2 font-serif">登录以继续</h2>
                <p class="text-sm text-slate-600">选择您的邮箱以访问聊天</p>
            </div>
            ...
            <div class="mt-5 p-3 bg-vanna-teal/10 border-l-4 border-vanna-teal rounded text-xs text-vanna-navy leading-relaxed">
                <strong>演示模式：</strong>这是一个仅前端的身份验证演示。
                您的邮箱将作为 Cookie 存储，并随所有 API 请求自动发送。
            </div>
        </div>
        ...
        <div id="loggedInStatus" class="hidden text-center p-4 bg-vanna-teal/10 border border-vanna-teal/30 rounded-lg mb-5">
            已登录为 <span id="loggedInEmail" class="font-semibold text-vanna-navy"></span>
            <br>
            ...
            <div class="mt-8 p-5 bg-white rounded-lg shadow border border-vanna-teal/30">
                ...
                <ul class="space-y-2">
                    <li class="p-2 bg-vanna-cream/50 rounded font-mono text-sm">
                        <span class="font-bold text-vanna-teal mr-2">POST</span>{api_base_url}/api/vanna/v2/chat_sse - 服务器发送事件流式传输
                    </li>
                    <li class="p-2 bg-vanna-cream/50 rounded font-mono text-sm">
                        <span class="font-bold text-vanna-teal mr-2">WS</span>{api_base_url}/api/vanna/v2/chat_websocket - WebSocket 实时聊天
                    </li>
                    <li class="p-2 bg-vanna-cream/50 rounded font-mono text-sm">
                        <span class="font-bold text-vanna-teal mr-2">POST</span>{api_base_url}/api/vanna/v2/chat_poll - 请求/响应轮询
                    </li>
                    <li class="p-2 bg-vanna-cream/50 rounded font-mono text-sm">
                ...
                vannaChat.addEventListener('artifact-opened', (event) => {{
                    ...
                    setTimeout(() => {{
                        ...
                        if (newWindow) {{
                            ...
                            console.log(`📱 已在新窗口中打开 ${{title}}`);
                }}, 100);

                // Prevent default in-chat rendering
                event.detail.preventDefault();
                console.log('✋ 在聊天中显示占位符而不是完整作品');
            }});

            console.log('🎯 作品演示模式：所有作品将在外部打开');
        }}
    }});
            ...
            if (!customElements.get('vanna-chat')) {{
                document.querySelector('vanna-chat').innerHTML = `
                        <div class="p-10 text-center text-gray-600">
                            <h3 class="text-xl font-semibold mb-2">Vanna 聊天组件</h3>
                            <p class="mb-2">Web 组件加载失败，请检查您的网络连接。</p>
                            <p class="text-sm text-gray-400">
                                {("加载来源：本地静态资源" if dev_mode else f"加载来源：{cdn_url}")}
                            </p>
                        </div>
                    `;
            }}
